...
export default function CalendarPage() {
  ...
  async function refresh() {
    setLoading(true)
    try {
      const data = await appointmentsApi.listByMonth(cursor.year, cursor.month)
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="bg-violet-600 hover:bg-violet-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-1.5 transition shadow-sm"
          >
          ...
          {cells.map((day, i) => (
            <div
              ...
              className={`min-h-[110px] border-r border-b border-gray-100 p-1.5 ${
                !day ? 'bg-gray-50/50' : 'hover:bg-violet-50/30 cursor-pointer transition'
              }`}
              onClick={() => {
                if (!day) return
              ...
              {day && (
                <>
                  <div className={`text-xs font-semibold mb-1 ${
                    isToday(day) ? 'bg-violet-600 text-white w-6 h-6 rounded-full flex items-center justify-center' : 'text-gray-600'
                  }`}>
                    {day}
                  </div>
                    ...
                    {(apptByDay[day] || []).slice(0, 3).map(a => {
                      ...
                      return (
                        <button
                          ...
                          title={`${a.clientName}${a.service ? ` — ${a.service}` : ''}`}
                        >
